...
def test_basic_state_management():
    ...
    # Add states
    _ = manager.add_state("login", "Login Screen")
    _ = manager.add_state("main_menu", "Main Menu")
    _ = manager.add_state("editor", "Code Editor")
...
def test_groups():
    ...
    manager.add_transition("open_workspace_ui", activate_groups=["workspace_ui"])
...
def test_pathfinding_integration():
    ...
    config = StateManagerConfig(default_search_strategy=SearchStrategy.DIJKSTRA)
...
def test_history_tracking():
    ...
    assert manager.transition_history[0][1] is True
    assert manager.transition_history[1][0] == "b_to_c"
    assert manager.transition_history[1][1] is True
    assert manager.transition_history[2][0] == "invalid"
    assert manager.transition_history[2][1] is True
...
def test_complex_scenario():
    ...
    manager.add_transition(
        "open_editor", from_states=["main_window"], activate_states=["code_editor"]
    )
    ...
    manager.add_transition(
        "show_settings", from_states=["main_window"], activate_states=["settings"]
    )
    ...
    if path:
        ...
        print(
            f"Dev tools active: {manager.is_active('terminal')}, "
